...
#sends mails in pptx format
def send_email( attachment_path,receiver_email):
    sender_email = 'sendersmail'
    ...
    with open(attachment_path, 'rb') as attachment:
        message.add_attachment(attachment.read(), maintype='application', subtype='vnd.openxmlformats-officedocument.presentationml.presentation', filename=attachment_path)
...
class CertificateGenerator:
    ...
    def merge(self):
        ...
        df = pd.read_excel(self.excel_path.get().split(": ")[1])#[1] for getting the path of excel
        emailaddress=df["email"].tolist()
        for index, row in df.iterrows():
            ...
            modified_pptx_file = os.path.join(destination_folder, 'certificate.pptx')
            modified_presentation.save(modified_pptx_file)



            attachment_path=modified_pptx_file
            send_email(attachment_path,emailaddress[index])
